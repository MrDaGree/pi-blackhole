...
- **Deterministic 4xx errors now cool models down and engage fallbacks.** A 400-class failure (missing provider-required headers, bad credentials, unknown model) previously retried the same broken model on every consolidation cycle because only transient 429/5xx/timeout counted as cooldown-worthy — and the session model (no candidate config) never recorded cooldown at all. New `isDeterministicError` / `isCooldownWorthyError` classification; deterministic failures persist cooldown (default 1h) including for the resolved session model, so the stage falls through to `*FallbackModels` instead of burning attempts. Transient blips on the main model still only trip the 30s retry gate, never an hour-long cooldown.
