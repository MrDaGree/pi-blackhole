...
// Spinner frames advance every 120ms via setInterval; pi's footer repaint
// loop may drop frames under load.
...
export function registerStatusBar(pi: ExtensionAPI, runtime: Runtime): void {
  ...
  function tick(): void {
    // Session branches grow by appending at the tip and never rewrite history
    // (see the entryIndexById cache comment in ledger/progress.ts), so
    // length + tail ID suffice for change detection.
    if (
      lastCtx &&
